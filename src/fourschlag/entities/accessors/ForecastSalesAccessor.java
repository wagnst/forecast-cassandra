...
public interface ForecastSalesAccessor {
    @Query("SELECT sales_volumes, net_sales, cm1 FROM forecast_sales WHERE product_main_group = :product_main_group AND period = :period AND plan_year = :plan_year AND region = :region AND sales_type = :sales_type AND entry_type = :entry_type ALLOW FILTERING;")
