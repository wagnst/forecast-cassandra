...
public interface ActualSalesAccessor {
    ...
    @Query("SELECT sales_volumes, net_sales, cm1 FROM actual_sales WHERE product_main_group = :product_main_group AND period = :period AND region = :region AND sales_type = :sales_type AND data_source = 'BW B' ALLOW FILTERING;")
